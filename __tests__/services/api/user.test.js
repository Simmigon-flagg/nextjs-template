...
describe("user service", () => {
  ...
  describe("updateUserByEmail", () => {
    it("returns null if user not found", async () => {
      ...
      const result = await updateUserByEmail("noone@example.com", { name: "New" });
  ...
  describe("getUserProfile", () => {
    ...
    it("returns user profile with image URL if image found", async () => {
      ...
      const bucketFind = jest.fn().mockReturnValue({ toArray: jest.fn().mockResolvedValue(mockImage) });
  ...
  describe("resetPassword", () => {
    it("throws error if missing token or newPassword", async () => {
      await expect(resetPassword(null, "pass")).rejects.toMatchObject({ status: 400 });
      await expect(resetPassword("token", null)).rejects.toMatchObject({ status: 400 });
    });
  ...
  describe("requestPasswordReset", () => {
    it("throws error if email missing", async () => {
      await expect(requestPasswordReset(null)).rejects.toMatchObject({ status: 400 });
    });
    ...
    it("creates reset token, saves user and sends email", async () => {
      ...
      expect(sendEmail).toHaveBeenCalledWith(
        ...
        expect.stringContaining("https://example.com/reset-password?token=resettoken")
      );
    });
  });
});
